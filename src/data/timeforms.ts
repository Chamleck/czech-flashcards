...
// стандартний спосіб; Naše řeč (ÚJЧ) — корпусні межі «večer» 18–22 з піком ~20.
// Точні межі формально розмиті (сам ÚJЧ це визнає) — тому у квізі використовуємо
// лише "безпечні" години з середини кожного проміжку, подалі від межі, щоб
// не створити новий спірний кейс на кшталт tisíc/tisíci.
export function dayPart(h24: number): string {
  if (h24 >= 6 && h24 < 9) return "ráno";
  if (h24 >= 9 && h24 < 12) return "dopoledne";
  if (h24 >= 12 && h24 < 18) return "odpoledne";
  if (h24 >= 18 && h24 < 22) return "večer";
  return "v noci";
}
