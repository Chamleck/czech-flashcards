...
import { RootStackParamList, CardProgress, SpatialAdverbEntry } from "../types";
...
// Прислівники місця — незмінна частина мови, тому окрема сесія (не WordSession/
// DeclSession), той самий механізм, що PrepositionSessionScreen: показати
...
export function AdverbSessionScreen({ route, navigation }: Props) {
  ...
  const entries = useMemo(
    () => ADVERBS.filter((a) => entryIds.includes(a.id)),
    [entryIds]
  );
  ...
  useEffect(() => {
    loadProgressFrom(PROGRESS_KEYS.adverbs).then((p) => {
      setProgress(p);
      setLoaded(true);
    });
  }, []);
  ...
  async function answer(knewIt: boolean) {
    ...
    setProgress(updated);
    await saveProgressTo(PROGRESS_KEYS.adverbs, updated);
  ...
  return (
    <View style={styles.safe}>
      <View style={styles.cardArea}>
        <AdverbCard entry={current} revealed={revealed} onReveal={() => setRevealed(true)} />
      </View>
